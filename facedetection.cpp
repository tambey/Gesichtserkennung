...
/** @function LoadHaarClassifier */
/** Load the defined Cascade Classifiers */
bool LoadHaarClassifier() {

    if( !eyeglasses_cascade.load( eyeglasses_cascade_name ) ) {
        cout << "\nERROR: Could not load eye classifier!" << endl;
        cascade = false;
        return -1 ;
    };

    if( !face_cascade.load( face_cascade_name ) ) {
        cout << "\nERROR: Could not load face classifier!" << endl;
        cascade = false;
        return -1;
    };

    if( !mouth_cascade.load( mouth_cascade_name ) ) {
        cout << "\nERROR: Could not load mouth classifier!" << endl;
        cascade = false;
        return -1;
    };
    return cascade;
...
void detectAndDisplay( Mat frame ) {
    ...
    if(!(faces.size() == 0)) {
        ...
        for( unsigned int i = 0; i < faces.size(); i++ ) {
            ...
            imwrite(g.str(), faceROI);      /** save detected faces */

            /** write the detected faces in Record.avi */
            record << frame;
  ...
  /** Show what you got in GUI */
  imshow( window_name, frame );
 }

/** @function Send_Email */
/** Use This function  if you want to send an E-Mail to user*/
int Send_Email() {
	 CURL *curl;				/** curl */
